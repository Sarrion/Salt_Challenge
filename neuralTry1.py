...
W1 = tf.Variable(tf.random_normal(shape = [10201, 10201]))
b1 = tf.Variable(tf.random_normal(shape = [10201]))
#W2 = tf.Variable(tf.random_normal(shape = [10201, 10201]))
...
loss = tf.reduce_mean(tf.losses.sigmoid_cross_entropy(layer1, y_target))   
...
for i in range(len(x_vals_train)):
    sess.run(train_step, feed_dict = {x_data: x_vals_train[i,:],
                                      y_target: y_vals_train[i,:]})
    temp_loss = sess.run(loss, feed_dict = {x_data: x_vals_train[i,:],
                                            y_target: y_vals_train[i,:]})
    loss_vec.append(np.sqrt(temp_loss))
    test_temp_loss = sess.run(loss, feed_dict = {x_data: x_vals_train[i,:],
                                                 y_target: y_vals_train[i,:]})
